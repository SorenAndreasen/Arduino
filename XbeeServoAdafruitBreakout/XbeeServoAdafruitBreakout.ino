...
long interval = 150; // tiden der går fra at bell servoerne trækker sig tilbage, i millisekunder
...
void loop() {
  ...
  // debug:
   ...
   if(incomingByte[0] == 0) // clock 0
   {
     ...
     else if(incomingByte[1] == 2) triggerBell(0, incomingByte[2]); // trigger bell
   ...
   else if(incomingByte[0] == 1) // clock 1
   {
     ...
     else if(incomingByte[1] == 2) triggerBell(1, incomingByte[2]); // trigger bell
   ...
   else if(incomingByte[0] == 2) // clock 2
   {
     ...
     else if(incomingByte[1] == 2) triggerBell(2, incomingByte[2]); // trigger bell A
     else if(incomingByte[1] == 3) triggerBell(3, incomingByte[2]); // trigger bell B
...
void triggerBell(int bell, int vel) 
{
  if(bell == 0) // clock 0
  {
    Serial.println("bell0Triggered");
    previousBellHit[0] = currentMillis;
    uint16_t pulselen = map(vel, 0, 180, BELLMIN, BELLMAX);
    pwm.setPWM(2, 0, pulselen); 
    bellActive[0] = 1;
  }
